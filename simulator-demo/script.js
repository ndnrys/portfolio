...
(()=> {
    const ansinWidgetUtil = {
        ...
        getItemsHTML: ((area, obj, idx) => {
            ...
            const items = obj.items.map((item, iidx) => {
                ...
                if (obj.type === 3) {
                    input += label;
                    input += `<input type="number" name="ansinWidget-${area}${propIdx}" class="ansinWidget-number ansinWidget-calc" value="0" min="${item.min? item.min: 0}" ${item.max?  "max=" + item.max: ""}  data-price="${item.price}" id="ansinWidget-${area}${propIIdx}"></input>`;
                }
                return `<div class="ansinWidget-question">${input}</div>`;
            });
            const select = (() => {
                if (obj.type !== 2) return "";
                const options = obj.items.map((item) => `<option value="${item.price}">${item.name}</option>`);
                return `<div class="ansinWidget-question"><select name="ansinWidget-${area}-select${propIdx}" class="ansinWidget-select ansinWidget-calc" id="ansinWidget-${area}-select_${idx}"><option value="">選択してください</option>${options.join("")}</select></input></div>`;
            });
            
            return `<div class="ansinWidget-${area}-inner" ${typeof idx === "undefined"? "": 'data-idx="' + idx + '"'} data-required="${obj.required}">
        ...
        calcSimulation: ((e) => {
            ...
            const totalPriceElement = widget.querySelector(".ansinWidget-total-price");
            ...
            allWrappers.forEach(wrapper => {
                ...
                if (numberInput) {
                    ...
                    if (required) { 
                        if (count > 0 || (numberInput.min !== null && parseInt(numberInput.min) === 0 && count >= 0)) {
                             isSelected = true;
                        }
                    } else if (count >= 0) {
                        // 必須でない項目は0以上であればOK
                ...
                if (isUserInteraction && wrapper === targetWrapper) {
                    if (required && !isSelected) {
                        ...
                        if (errorElement) errorElement.textContent = ansinWidgetUtil.texts.requiredError;
                    } else {
            ...
            if (globalIsValid) {
                ...
                if (totalPriceElement) totalPriceElement.textContent = `¥${formattedTotal} 円`;
            } else {
                // ユーザー操作時のみ、全体エラーメッセージを表示
                if (isUserInteraction) {
                    if (totalTxtElement) totalTxtElement.textContent = ansinWidgetUtil.texts.selectionError;
                } else {
                    // 初期表示時はテキストも価格も空欄にする
    ...
    const ctaUrl = ansinWidgetConfig.cta.url.startsWith('http') ? ansinWidgetConfig.cta.url : `http://${ansinWidgetConfig.cta.url}`;
    const cta = ansinWidgetConfig.cta.use? `<div class="ansinWidget-cta"><a href="${ctaUrl}" class="ansinWidget-cta-link">${ansinWidgetConfig.cta.text}</a></div>`: "";
    const base = `<div class="ansinWidget-base">${ansinWidgetUtil.getItemsHTML("base", ansinWidgetConfig.base)}</div>`;
    const options = ansinWidgetConfig.option.map((op, idx) => ansinWidgetUtil.getItemsHTML("option", op, idx));
    ...
    if (!widget) return;
        ...
        calcs.forEach(calc => calc.addEventListener("change", ansinWidgetUtil.calcSimulation));
